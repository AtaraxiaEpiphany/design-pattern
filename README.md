...
> 责任链模式允许多个对象按照顺序处理请求，直到其中一个对象能够处理该请求为止。
> 责任链模式将请求发送者和接收者解耦,使得多个对象都有机会处理请求,同时避免了请求发送者与接收者之间的直接耦合关系。
>
> 主要角色:

- 抽象处理者（Handler）：定义了处理请求的接口，通常包含一个处理方法（handle），用于处理请求。
  抽象处理者可以定义一个`后继处理者(successor)`属性，用于将请求传递给下一个`处理者`。

- 具体处理者（Concrete Handler）：实现了抽象处理者接口，具体处理者负责处理请求。如果能够处理请求，则处理请求；否则将请求传递给下一个处理者。

### `状态模式(State Pattern)`

> 状态模式允许对象在内部状态改变时改变其行为。该模式将对象的行为封装在不同的状态类中，使得对象在不同状态下可以有不同的行为。
>
> 主要角色:

- 上下文（Context）：上下文是包含`状态`的对象，它维护一个对当前`状态对象`
  的引用，并将客户端的请求委派给当前状态对象处理。上下文可以定义一些与`状态`相关的`方法`，供状态对象调用。
- 抽象状态（Abstract
  State）：抽象状态是一个接口或抽象类，定义了具体状态类需要实现的方法。这些方法通常与上下文的请求相关，并且可能会`改变上下文的状态`
  。抽象状态可以包含一些默认实现，以便在具体状态类中进行重用。
- 具体状态（Concrete State）：具体状态是抽象状态的实现类，它实现了在特定状态下的行为。每个具体状态类都负责处理特定状态下的请求，并且可能会改变上下文的状态。具体状态类可以包含一些与该状态相关的数据和逻辑。

## `观察者模式(Observer Pattern)`

> 观察者模式定义了一种一对多的依赖关系，
> 使得多个观察者对象可以同时监听和被通知一个主题对象的状态变化。
>
> 主要角色:

- 主题（Subject）：也称为被观察者或可观察对象。主题维护`一组观察者对象`
  ，并提供方法来添加、删除和通知观察者。主题通常包含一个状态，当状态发生变化时，会通知所有`注册的观察者`。
- 具体主题（Concrete Subject）：具体主题是主题的具体实现类。它维护了一个`观察者列表`，并在状态变化时`通知所有注册的观察者`
  。具体主题通常包含特定的业务逻辑，用于改变状态并通知观察者。
- 观察者（Observer）：观察者是依赖于主题的对象，它们通过`注册到主题上`
  来接收状态变化的通知。观察者定义了一个更新方法，当主题状态发生变化时，该方法会被调用。观察者可以根据需要进行自定义操作。
- 具体观察者（Concrete Observer）：具体观察者是观察者的具体实现类。它实现了观察者接口中定义的更新方法，以便在接收到主题通知时执行相应的操作。具体观察者可以根据需要定义自己的状态和行为。

## `中介者模式(Mediator Pattern)`

> 中介者模式通过封装`一系列对象`之间的交互，将对象之间的通信集中处理，从而降低对象之间的耦合性。
> 对象之间不直接相互通信，而是通过一个中介者对象进行通信。中介与对象形成了星型关系,中介者处于最中央.(
> 类似于注册中心与发现中心).
> 中介者对象负责协调和管理对象之间的交互，它知道每个对象的存在并了解它们的行为。当一个对象需要与其他对象进行通信时，它将请求发送给中介者，然后由中介者将请求转发给适当的对象。

> 主要角色:

- 中介者（Mediator）：中介者是一个接口或抽象类，定义了对象之间的通信接口。
  它通常包含将请求转发给其他对象的方法。中介者知道所有的`同事对象`，并负责协调它们之间的交互。
- 具体中介者（Concrete
  Mediator）：具体中介者是中介者接口的实现类。它实现了中介者定义的通信接口，并负责协调和管理同事对象之间的交互。具体中介者通常持有`对所有同事对象的引用`
  ，并根据接收到的请求将其转发给`适当的同事对象`。
- 同事对象（Colleague）：同事对象是相互之间需要通信的对象。每个同事对象都知道`中介者对象`
  ，并通过中介者对象与其他同事对象进行通信。同事对象之间的通信通过中介者对象进行转发。
- 具体同事对象（Concrete
  Colleague）：具体同事对象是同事对象接口的实现类。它实现了同事对象的行为，并通过中介者对象与其他同事对象进行通信。具体同事对象通常包含一些自身的业务逻辑，同时也依赖于中介者对象来处理与其他同事对象的通信。

## 迭代器模式(Iterator Pattern)

> 迭代器模式提供一种顺序访问聚合对象中各个元素的方法，而无需暴露聚合对象的内部表示。
>
> 主要角色:

- 迭代器接口（Iterator）：定义了访问和遍历聚合对象元素的方法，包括获取下一个元素、判断是否还有元素等操作。
- 具体迭代器（Concrete Iterator）：实现迭代器接口，负责实现具体的遍历算法。它追踪聚合对象的当前位置，并根据遍历算法返回正确的元素。
- 聚合对象接口（Aggregation）：定义了创建`迭代器对象`的方法(Iterable)。聚合对象可以是一个集合、列表、数组或其他可遍历的数据结构。
- 具体聚合对象（Concrete Aggregation）：实现聚合对象接口，负责创建具体的迭代器对象。它提供了具体的聚合对象，供迭代器使用。

## 访问者模式(Visitor Pattern)

> 访问者模式允许在不改变被访问对象的类的前提下，定义新的操作（访问者）并应用于这些对象。
> 核心思想是将数据结构与对数据的操作分离开来。
> 它通过定义一个访问者接口和多个具体访问者类，以及一个被访问对象接口和多个具体被访问对象类，实现了对被访问对象的不同操作。
>
> 主要角色:

- 访问者接口（Visitor）：定义了访问者的操作方法，每个方法对应一个`具体被访问对象`
  的操作。访问者接口中的方法通常以`被访问对象`接口中的方法作为参数，以便访问者可以通过不同的操作访问被访问对象。
- 具体访问者（Concrete Visitor）：实现了访问者接口，具体定义了`对被访问对象`的操作。每个具体访问者类实现了一组特定的操作，可以对被访问对象进行不同的处理。
- 被访问对象接口（Element）：定义了被访问对象的接口，包括接受`访问者`的方法。被访问对象接口中的方法通常将自身作为参数传递给访问者，以便访问者可以对其进行操作。
- 具体被访问对象（Concrete Element）：实现了被访问对象接口，具体定义了接受访问者的方法。每个具体被访问对象类可以根据需要实现不同的接受访问者的逻辑。
- 对象结构（Object Structure）：包含`被访问对象`的集合，提供了遍历集合中的元素的方法。对象结构可以是一个集合、列表、树等数据结构。

#### 静态分派(Static Dispatch)

> 静态分派是在编译时确定方法调用的过程。
> 它是根据方法的参数静态类型（声明时的类型）来决定调用哪个方法。
> 在静态分派中，编译器根据参数的静态类型选择合适的方法。
> 静态分派通常发生在编译时期，因此也称为早期绑定。静态分派适用于方法重载的情况，
> 其中多个方法具有相同的名称但参数类型不同。

#### 动态分派(Dynamic Dispatch)

> 动态分派是在运行时确定方法调用的过程。
> 它是根据方法的实际类型（运行时的类型）来决定调用哪个方法。
> 在动态分派中，运行时系统根据对象的实际类型选择合适的方法。
> 动态分派通常发生在运行时期，因此也称为晚期绑定。
> 动态分派适用于方法重写的情况，其中子类重写了父类的方法。

## 备忘录模式(Memento Pattern)

> 备忘录模式允许在不破坏封装性的前提下捕获一个对象的内部状态，并在该对象之外保存这个状态(快照)，
> 以便在需要时恢复对象到原先的状态。
>
> 主要角色:

- 发起人（Originator）：发起人是需要被保存状态的对象。它拥有内部状态，并能够创建备忘录对象来保存当前状态。
  发起人还可以从`备忘录对象`中恢复到之前保存的状态。发起人通常是实现了备忘录模式的类或对象。
- 备忘录（Memento）：备忘录是用于存储`发起人对象状态的对象`
  。它能够捕获并保存发起人对象的内部状态，但对外部是不可见的。备忘录对象通常只提供了获取状态和设置状态的方法，而不暴露具体的状态细节。
- 管理者（Caretaker）：管理者是负责保存和恢复`备忘录对象的对象`。它可以存储多个`备忘录对象`，
  并在需要时将备忘录对象返回给发起人对象，以便发起人对象可以恢复到之前的状态。管理者对象通常是与发起人对象分离的独立对象，用于管理备忘录对象的集合。

## 解释器模式(Interpreter Pattern)

> 解释器模式用于定义一种语言的文法规则，并解释该语言中的表达式。
> 该模式通过定义表达式的类层次结构和解释方法，使得可以解释和执行特定语言的表达式。
>
> 主要角色:

- 抽象表达式（Abstract Expression）：抽象表达式定义了解释器模式中所有表达式的共同接口。它通常是一个抽象类或接口，声明了解释方法（interpret）的签名，具体的表达式类需要实现这个接口。
- 终结符表达式（Terminal Expression）：终结符表达式是解释器模式中的`基本表达式单元`，它`实现了抽象表达式接口`，并提供了具体的解释方法。终结符表达式通常表示语言中的基本元素，例如变量、常量或操作符。
- 非终结符表达式（Non-terminal Expression）：非终结符表达式是由多个终结符表达式组成的复合表达式，它也`实现了抽象表达式接口`，并提供了具体的解释方法。非终结符表达式通常表示语言中的复杂语法规则，例如语句、表达式或语法树。
- 上下文（Context）：上下文包含了解释器需要的全局信息或状态。它可以提供给解释器进行解释和执行表达式所需的上下文信息。
