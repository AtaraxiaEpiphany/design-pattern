...
- 抽象处理者（Handler）：定义了处理请求的接口，通常包含一个处理方法（handle），用于处理请求。抽象处理者可以定义一个`后继处理者(successor)`
属性，用于将请求传递给下一个`处理者`。
...
- 上下文（Context）：上下文是包含`状态`的对象，它维护一个对当前`状态对象`的引用，并将客户端的请求委派给当前状态对象处理。上下文可以定义一些与`状态`相关的`方法`，供状态对象调用。
- 抽象状态（Abstract State）：抽象状态是一个接口或抽象类，定义了具体状态类需要实现的方法。这些方法通常与上下文的请求相关，并且可能会`改变上下文的状态`。抽象状态可以包含一些默认实现，以便在具体状态类中进行重用。
